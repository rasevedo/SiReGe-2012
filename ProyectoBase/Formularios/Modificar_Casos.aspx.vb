...
Public Class Modificar_Casos
    ...
    Private Sub GetCaso()
        ...
        Dim cmd As SqlCommand = New SqlCommand("SELECT C.idCasos, C.numeroCasos, C.estadoCasos, C.fechaCasos, C.cedulaDenuncianteCasos, C.nombreDenucianteCasos, E.nombreEmpleados, C.nombreCentroEducativo, U.descripcionUnidad, U.descripcionDespacho, U.descripcionDireccion, U.descripcionDepartamento, C.numeroOficio, C.fechaOficio, D.tipoDimension, D.letraDimension, D.descripcionTipoDimension, C.condicionCasos, C.detalleInconformidadCasos, C.respuestaCasos, C.valoracionAdmisibilidad, C.veredictoValoracionIngreso, C.trazabilidadCasos, C.fechaRespuestaCasos, C.fechaCerradoCasos FROM casos C join Empleados as E on C.idEmpleados = E.idEmpleados join Unidad as U on C.idUnidad = U.idUnidad join Dimensiones as D on C.idDimension = D.idDimension  WHERE idCasos=@idCasos", con)
    ...
    Sub FiltrarPorCaso()
        ...
        Dim cmd As SqlCommand = New SqlCommand("SELECT ac.idAvanceCasos, c.idCasos, ac.detalleAvance, ac.fechaAvance FROM AvanceCasos ac join Casos as c on ac.idCasos = c.idCasos WHERE c.idCasos='" & txtidCasos.Text & "'", con)
